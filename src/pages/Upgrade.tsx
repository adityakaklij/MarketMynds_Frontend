import { BadgeIndianRupee, Receipt, AlertCircle, User, Mail, MessageSquare, Sparkles, Tag } from "lucide-react";
...
// Special yearly plan at discounted price
const specialPlan: Plan = {
  title: "1 Year",
  price: 550,
  originalPrice: 1188,
  description: "Special offer: Full access for 1 year.",
  bonus: true,
  planId: "4"
};
...
export default function Upgrade() {
  ...
  const [selectedPlan] = useState<Plan>(specialPlan);
  ...
  const handlePlanSelect = () => {
    ...
    window.dataLayer.push({
      ...
      ecommerce: {
        items: [{
          item_name: specialPlan.title,
          item_category: 'Subscription Plan',
          price: specialPlan.price
        }]
      }
    });
  ...
  return (
    <div id="upgrade" className="min-h-screen bg-black text-white pt-24 px-4 pb-20">
      {/* Add animation styles */}
      <style dangerouslySetInnerHTML={{ __html: animationStyles }} />
      
      <div className="max-w-2xl mx-auto text-center">
        <h1 className="text-4xl font-bold mb-1">Exclusive Upgrade Offer</h1>
        <p className="mb-8 text-gray-300 text-lg">Special yearly plan just for you. No auto-renewals. 3-day refund window.</p>
      </div>
      
      {/* Special plan card */}
      <div className="max-w-md mx-auto mb-12">
        <div className="rounded-xl border-2 border-finance-green/30 bg-gradient-to-b from-finance-blue/5 to-black p-8 flex flex-col items-center shadow-lg shadow-black/20">
          <div className="flex items-center gap-2 mb-2">
            <BadgeIndianRupee className="text-finance-green" />
            <span className="text-2xl font-semibold">{specialPlan.title}</span>
            <span className="ml-2 px-2 py-1 rounded bg-finance-green/10 text-finance-green text-xs font-medium">Special Offer</span>
          </div>
          
          <div className="flex flex-col items-center">
            <div className="relative mb-1">
              <span className="text-xl font-bold text-gray-400">₹{specialPlan.originalPrice}</span>
              <span className="absolute top-1/2 left-0 right-0 h-0.5 bg-red-500 transform -rotate-12"></span>
            </div>
            <div className="text-3xl font-extrabold text-finance-green mb-2">₹{specialPlan.price}</div>
            <span className="text-xs font-medium bg-finance-green/10 text-finance-green py-0.5 px-2 rounded-full mb-1">
              {Math.round(((specialPlan.originalPrice - specialPlan.price) / specialPlan.originalPrice) * 100)}% OFF
            </span>
          </div>
          
          <div className="text-gray-400 mb-6">{specialPlan.description}</div>
          ...
          <div className="my-6 text-center">
            {discountedPrice > 0 ? (
              ...
              <div className="flex flex-col items-center">
                <div className="inline-flex items-center gap-3 mb-1">
                  <div className="relative">
                    <span className="text-2xl font-bold text-gray-400">₹{specialPlan.originalPrice}</span>
                    <span className="absolute top-1/2 left-0 right-0 h-0.5 bg-red-500 transform -rotate-12"></span>
                  </div>
                  <span className="text-3xl font-extrabold text-finance-green">₹{specialPlan.price}</span>
                  <span className="text-xs font-medium bg-finance-green/10 text-finance-green py-0.5 px-2 rounded-full">
                    {Math.round(((specialPlan.originalPrice - specialPlan.price) / specialPlan.originalPrice) * 100)}% OFF
                  </span>
                </div>
